angular
	...
	.factory('AppSyncService', function($q, SyncService, $window) {
		...
		var _syncLocalForageObject = function (key, properties, runtimeObject) {
				...
				localforage.getItem(key, function(localforageObject, error) {
					...
					SyncService.syncObject(localforageObject, key, properties).then(function(syncResult) {

						// Update local object with result of sync
						localforage.getItem(key, function(localObject, error) {

							for (var prop in syncResult) {
								console.log(prop, localObject[prop], syncResult[prop]);
		...
		var self = {
			...
			syncLocalForageCustomWorkouts: function() {
				...
				localforage.getItem(LF_CUSTOM_WORKOUTS_KEY, function(localforageObject, error) {
					...
					SyncService.syncArray(customWorkoutsArray, LF_CUSTOM_WORKOUTS_KEY).then(function(syncResult) {
            ...
            tx.executeSql("SELECT * FROM Sworkit", [], function(tx, results) {
            	...
            	SyncService.syncArray(localWorkoutsArray, WEBSQL_WORKOUTS_LOG_KEY, 'sync_id').then(function(syncResult) {
								...
								syncResult.forEach(function (remoteWorkout) {
									...
									if ( !localWorkout ) {
										...
										db.transaction(function(tx) {
											tx.executeSql('INSERT INTO Sworkit(sync_id, sync_lastUpdated, created_on, minutes_completed, calories, type, utc_created, exercise_list) VALUES (?,?,?,?,?,?,?,?)',
																		[remoteWorkout.$id, remoteWorkout.sync_lastUpdated, remoteWorkout.created_on, remoteWorkout.minutes_completed, remoteWorkout.calories, remoteWorkout.type, remoteWorkout.utc_created, remoteWorkout.exercise_list], function(tx, success) { console.log('success', success) }, function(tx, error) {console.log('error', error)});
										});
										...
										db.transaction(function(tx) {
											tx.executeSql('UPDATE Sworkit SET sync_lastUpdated=?, created_on=?, minutes_completed=?, calories=?, type=?, utc_created=?, exercise_list=? WHERE sync_id=?',
																			[remoteWorkout.sync_lastUpdated, remoteWorkout.created_on, remoteWorkout.minutes_completed, remoteWorkout.calories, remoteWorkout.type, remoteWorkout.utc_created, remoteWorkout.exercise_list, remoteWorkout.$id], function(tx, success) { console.log('success', success) }, function(tx, error) {console.log('error', error)});
										});
									}
								});
								...
								localWorkoutsArray.forEach(function (localWorkout) {
									...
									if ( !localWorkout.sync_id || !_.has(syncResultSyncIdRegister, localWorkout.sync_id) ) {
										...
										db.transaction(function(tx) {
											tx.executeSql('DELETE FROM Sworkit WHERE sworkit_id = ?', [localWorkout.sworkit_id], function(tx, success) { console.log('success', success) }, function(tx, error) {console.log('error', error)});
										});
									}
								});
