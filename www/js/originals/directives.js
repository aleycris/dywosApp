...
.directive('multiBg', function(_){
	return {
		...
		controller: function($scope, $element, $attrs) {
			...
			this.animateBg = function(){
				...
				$scope.$apply(function () {
					$scope.loaded = true;
					$element.parent().css({'background-image': 'url(' + $scope.bg_img + ')'});
					$element.parent().addClass('login-background');
		...
		templateUrl: 'templates/login/partials/multi-bg.html',
		restrict: 'A',
		transclude: true
	};
})
...
.directive('showHideContainer', function(){
	return {
		...
		templateUrl: 'templates/login/partials/show-hide-password.html',
		restrict: 'A',
		replace: false,
		transclude: true
	};
