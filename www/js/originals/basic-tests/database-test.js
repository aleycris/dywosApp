    ...
    var createNextRow = function() {
        if (increment !== numberOfRows + 1){
            ...
            window.db.transaction(function(transaction) {
             transaction.executeSql('INSERT INTO Sworkit(created_on, minutes_completed, calories, type, utc_created) VALUES (?,?,?,?,?)',[useDate, timeToAdd, burn, typeId, useDate], nullHandler,errorHandler, createNextRow());
            });
         }
    }
